...
```{r, include=FALSE}
here::i_am("sandbox/2021_report.Rmd")
```
...
```{r}
create_tables <- function(data, value_1, value_2, table_title){
  data %>%
    arrange(desc(value_1)) %>%
    head(5) %>%
    gt() %>%
    gt::fmt_number(columns = value_1, decimals = 0) %>%
    gt::fmt_number(columns = population, decimals = 0) %>%
    gt::fmt_percent(columns = value_2) %>%
    gt::tab_header(title = md(table_title))
}

us_cases_table <- create_tables(us_current, 
                                "cases",
                                "cases_per_pop",
                                "**Most Cases**")
us_cases_table
...
us_cases <- us_clean %>%
  ...
  select(state, population, cases, cases_per_pop) %>%
  arrange(desc(cases)) %>%
    head(5)
us_deaths <- us_clean %>%
  filter(date == date_data) %>%
  select(state, population, deaths, deaths_per_pop) %>%
  arrange(desc(deaths)) %>%
  head(5)

us_cases_table <- create_tables(us_cases, "cases", "cases_per_pop")
us_cases_table
us_deaths_table <- create_tables(us_deaths, "deaths", "deaths_per_pop")
us_deaths_table
```

```{r}
us_cases_per <-us_current %>%
  select(state, population, cases, cases_per_pop) %>%
  # change cases to cases_per_pop
  arrange(desc(cases_per_pop))
print(head(us_current))
```
...
```{r}
make_reactables <- function(data_set, var_1){
  # create reactable from data
  table <- reactable(data_set %>% 
                       # remove date column
                       select(!date
                              ),
                     columns = list(var_1 = colDef(name = var_1_title,
                                                   filterable = TRUE
                                                   ),
...
us_1 <- make_reactables(us_current)
```
  ...
  r_table <- reactable::reactable(data_set %>% 
                                  ...
                                  columns = base::list(
                                    ...
                                    partial_vacc_per_pop = reactable::colDef(name = "Percent Partially Vaccinated",
                                                                  format = reactable::colFormat(percent = TRUE,digit = 2)),
...
us_1 <- make_reactable(us_current, "state", "State")
```
...
reactable(us_current %>% select(!date),
          columns = list(
...
us.states.cases <- us_clean %>%
  ...
  arrange(desc(cases)) %>%
    ...
    fmt_percent(columns = cases_per_pop, decimals = 2) %>%
