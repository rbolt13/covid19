...
To begin lets look at the country as a whole, by state. The data will be filtered for **September 2nd, 2021**, and then lets look at the top five states with :
...
#   filter(date =="2021-09-02") %>%
#   select(state, population, cases, perc.cases) %>%
#   arrange(desc(cases)) %>%
#     head(5) %>%
#     gt() %>%
#     fmt_number(columns = cases, decimals = 0) %>%
#     fmt_number(columns = population, decimals = 0) %>%
#     fmt_percent(columns = perc.cases, decimals = 2) %>%
#     tab_header(title = md("**Most Cases**"),
#                subtitle = " as of September 2nd, 2021") %>%
#     cols_label(state = md("**State**"),
#                population = md("**Total Population**"),
#                cases = md("**Cases**"),
#                perc.cases = md("**Percentage**"))
# us.states.cases
```


* the highest percentage of cases per state 
```{r, echo=F}
# us.states.cases.perc <- us.states.vacc %>%
#   filter(date =="2021-09-01") %>%
#   select(state, population, cases, perc.cases) %>%
#   arrange(desc(perc.cases)) %>%
#     head(5) %>%
#     gt() %>%
#     fmt_number(columns = cases, decimals = 0) %>%
#     fmt_number(columns = population, decimals = 0) %>%
...
#                subtitle = " as of September 1st, 2021") %>%
#     cols_label(state = md("**State**"),
#                population = md("**Total Population**"),
#                cases = md("**Cases**"),
#                perc.cases = md("**Percentage**"))
# us.states.cases.perc
```
...
* the highest percentage of deaths per state
```{r, echo=F}
us.states.deaths.perc <- us.states.vacc %>%
  ...
  arrange(desc(perc.deaths)) %>%
    ...
    tab_header(title = md("**Highest percent of Deaths**"),
               subtitle = "By state, as of September 1st, 2021") %>%
    cols_label(state = md("**State**"),
               population = md("**Total Population**"),
               deaths = md("**Deaths**"),
               perc.deaths = md("**Percentage**"))
us.states.deaths.perc
```

* the most people fully vaccinated 
```{r, echo=F}
us.states.vacc <- us.states.vacc %>%
  filter(date =="2021-09-01") %>%
...
* the highst percentage of people fully vaccinated 
```{r, echo=F}
...
# us.states.vacc.perc
```
* the least amount of people fully vaccinated

* the lowest percentage of people fully vaccinated. 
